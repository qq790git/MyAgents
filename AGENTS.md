...
## Project Overview
- Game built with Cocos Creator 3.8.8 and TypeScript.
- Source code lives under `assets/Script/`.
- Scenes: `assets/Scene/Root.scene` 
...
- Creator version: 3.8.8 (match the project version to avoid asset serialization issues).
- TypeScript is strict (`tsconfig.json` enables strict + noImplicitAny).
- Engine APIs are imported from `cc`.

## Copilot / Cursor Rules /Roo Rules
...
### Formatting
- Indent with 4 spaces in most model classes (e.g. `GameModel.ts`).
...
- Open the repo in Cocos Creator 3.8.8 to run and verify UI changes.
